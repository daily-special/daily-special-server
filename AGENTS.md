...
- `docs/api.md`: **런타임 API 명세.** 클라이언트가 이 서버와 말하는 유일한 표면. 이 저장소가 소유한다
- `docs/data-contract.md`: 파이프라인·클라이언트와 공유하는 JSON 계약. **`api.md`와 다른 것이다** — 이쪽은 파이프라인 산출물이고, 5절이 런타임 상태를 명시적으로 제외한다
